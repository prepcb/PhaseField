...
subroutine app_mg_smooth(pid,level)
  ...
  double precision:: new_soln(il_bnd:iu_bnd,jl_bnd:ju_bnd,kl_bnd:ku_bnd,N_unk)
  double precision :: phi(n_phases)
  iopt=1
...
!     do lb=block_starts(level), block_starts(level+1)-1
     do lbs=block_starts(level), block_starts(level+1)-1
        ...
        call pf_mg_smooth_var(lb,new_soln,pid)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine pf_mg_smooth_var(lb,new_soln,pid)
  ...
  use tree
  implicit none
  integer,intent(in) :: lb,pid
  integer :: i,j,k,ii,aa,bb
  ...
  do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d-1,2 
     do i=il_bnd+nguard,iu_bnd-nguard-1,2
        ...
        do aa=1,4
           do ii=1,N_unk
              bb=bb+1
              Fi(bb) = Fi(bb) - unk(2+(ii-1)*nunkvbles,i+rrarray(aa,1),j+rrarray(aa,2),k,lb)
           enddo
        enddo
        ...
        do ii=1,N_unk
           do aa=1,4
              new_soln(i+rrarray(aa,1),j+rrarray(aa,2),k,ii)=unk(1+(ii-1)*nunkvbles,i+rrarray(aa,1),j+rrarray(aa,2),k,lb)-weight*Wk(ii+(aa-1)*N_unk)
           enddo
        enddo
     end do
  end do
        ...
        do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d 
           do i=il_bnd+nguard,iu_bnd-nguard
              ...
              do ii=1,N_unk
                 ...
                 !d = f - A(u)
                 unk(n+nunkvbles,i,j,1,lb) = unk(n+2,i,j,1,lb)  -Fi(ii)
                 Fi(ii)=unk(n+nunkvbles,i,j,1,lb) 
...
double precision :: unk_(M_unk),vblesd(N_unk+1,3,3),hh=1d-10
integer ip,n,ii,jj,aa
double precision :: FiM(N_unk,4),FidM(N_unk,4), unkM(M_unk,4),S
double precision :: vblesM(N_unk+1,3,3,4),c,rfactor,rf4,rf5,rf6,MolPerVol
...
enddo
  ...
  do ii=1,3
     do jj=1,3
        do aa = 1,4
           if(mode_1.eq.1) then
              vblesM(ip,ii,jj,aa)=(unk(1,i+ii-2+rrarray(aa,1),j+jj-2+rrarray(aa,2),1,lb)-&
                   unk(2,i+ii-2+rrarray(aa,1),j+jj-2+rrarray(aa,2),1,lb))/dt
           else
              rfactor = dt/dtold
              rf4 = rfactor*rfactor/(rfactor+1.0)
              rf5 = (rfactor+1.0)
              rf6 = (2.0*rfactor+1.0)/(rfactor+1.0)
              vblesM(ip,ii,jj,aa)=(rf4*unk(5,i+ii-2+rrarray(aa,1),j+jj-2+rrarray(aa,2),1,lb)-&
                   rf5*unk(4,i+ii-2+rrarray(aa,1),j+jj-2+rrarray(aa,2),1,lb)+&
                   rf6*unk(1,i+ii-2+rrarray(aa,1),j+jj-2+rrarray(aa,2),1,lb))/dt
           endif
              c=vblesM(N_unk,ii,jj,aa)
              vblesM(ip,ii,jj,aa)=-vblesM(ip,ii,jj,aa)*g_W(1)/((1.-c)+c*g_M(2)/g_M(1))
              MolPerVol=g_MolPerVol(1)*(1.-c)+g_MolPerVol(2)*c
              vblesM(ip,ii,jj,aa)=vblesM(ip,ii,jj,aa)/MolPerVol*g_lambda**2
        enddo
     enddo
  enddo
