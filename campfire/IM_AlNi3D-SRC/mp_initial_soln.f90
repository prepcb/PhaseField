...
! include files 
  ...
  integer :: n=999,nn=9999,inx,iny,inz
  double precision :: Gibbs_FE_l_AlSi,Gibbs_FE_e_AlSi,c_l,c_s,c_a,c_b,T_
...
!  d =  2*sqrt(8.)*g_lambda! * g_ratio
!  d =  1*sqrt(8.)*g_lambda! * g_ratio
  d = g_lambda*g_ratio
  g_width = 1d0*g_lambda !=1/(dphi/dx) at phi=1/2
           ...
           do i=il_bnd+nguard0,iu_bnd-nguard0
              ...
              do ii=1,nunkvbles-1
                 if(g_multiple)then
                    ...
                    unk(ii,i,j,k,lb)=1./(1.+exp(-(sqrt((xi)**2+(yi)**2+(zi)**2)-nuc_radius)/g_lambda)+&
                         exp(-(sqrt((xi-g_x0)**2+(yi-g_y0)**2+(zi-g_z0)**2)-2*nuc_radius)/g_lambda))

                 else
                    if(g_parallel_scaling)then
!                       write(*,*)g_grid, g_nbx,g_nby,g_nbz
                       unk(ii,i,j,k,lb)=1.
                       do inx = 1, g_nbx
                          g_x0 = 0.5* g_grid + (inx-1)*g_grid
                          do iny = 1,g_nby
                             g_y0 = 0.5* g_grid + (iny-1)*g_grid
                             do inz = 1,g_nbz
                                g_z0 = 0.5* g_grid + (inz-1)*g_grid
                                unk(ii,i,j,k,lb)=unk(ii,i,j,k,lb) + exp(-(sqrt((xi-g_x0)**2+(yi-g_y0)**2+(zi-g_z0)**2)-nuc_radius)/g_lambda)
                             enddo
                          enddo
                       enddo
                       unk(ii,i,j,k,lb) = 1./unk(ii,i,j,k,lb)
                    else
                       unk(ii,i,j,k,lb)=1./(1+exp(-(sqrt((xi-g_x0)**2+(yi-g_y0)**2+(zi-g_z0)**2)-nuc_radius)/g_lambda))
                    endif
                 endif
                 if(g_plapp)then

!!$                    if(unk(1,i,j,k,lb).lt.0.9)then
!!$                       unk(ii+nunkvbles,i,j,k,lb) = g_slope_factor
!!$                    else
!!$                       unk(ii+nunkvbles,i,j,k,lb) = (g_slope_factor-g_accel_factor*0.5*(g_AlFe_c(2,1)-g_AlFe_c(1,1)))
!!$                    endif

                       c_ave = g_mu_infinity*g_AlFe_c(1,1)+(1d0 - g_mu_infinity)*g_AlFe_c(2,1)!g_mu_infinity=1 is stronger
                       unk(ii+nunkvbles,i,j,k,lb) = (1-unk(ii,i,j,k,lb))*g_slope_factor&
                            +unk(ii,i,j,k,lb)* (g_slope_factor-g_accel_factor*(c_ave-g_AlFe_c(1,1)))
!                    unk(ii+nunkvbles,i,j,k,lb) = g_slope_factor-g_accel_factor*0.5*(g_AlFe_c(2,1)-g_AlFe_c(1,1))

                 else
                    
                    c_ave = g_infinity*g_AlFe_c(1,1)+(1d0 - g_infinity)*g_AlFe_c(2,1)
                    !                 unk(ii+nunkvbles,i,j,k,lb) = (1d0-unk(ii,i,j,k,lb))*g_AlFe_c(1,1)+unk(ii,i,j,k,lb)*c_ave
                    
                    
                    unk(ii+nunkvbles,i,j,k,lb) = (1d0-unk(ii,i,j,k,lb))*g_AlFe_c(1,1)&
                         +unk(ii,i,j,k,lb)*(c_ave + nuc_radius*(g_AlFe_c(2,1)-g_AlFe_c(1,1))*0.5/(g_lambda+sqrt((xi)**2+(yi)**2+(zi)**2)))
                 endif
              enddo
           enddo
        enddo
     ...
     write(*,*)"app_initial_soln_blk. Not set up for HP method"
     stop
!!$     do j=2,g_order-1
...
subroutine find_antitrapping_AlFe
  ...
  logical :: AdamsBashforth = .false.,phi_method=.true.
     ...
     return
        ...
        if(g_alpha.ne.0d0)then !else use input value 
           ...
           do jj=1,50
              ...
              g_c_max = g_AlFe_c(1,1)
              g_c_min = g_AlFe_c(2,1)
              ...
              do ii =2,nn
                 ...
                 f_cc =FreeEnergy(c_f(ii-1),g_T_i,phi_,-2,.true.)/(g_R*g_T0)
                 f_c_phi = FreeEnergy(c_f(ii-1),g_T_i,phi_,-1,.true.)/(g_R*g_T0)
                 ...
                 if(phi_method)then
                    ...
                    ff(ii) = (dh/f_cc)*(f_c_phi*(1d0+g_alpha*u*d/(g_D(1))*g_Dch)*d_phi&
                         +u*(c_f(ii-1)-g_c_max)/D_sol)
                 endif


                 c_f(ii) = c_f(ii-1) - ff(ii)

                 if(c_f(ii).lt.c_)then
                    c_ = c_f(ii)
                    if(phi_method)then
                       xx = h!phi_
                    else
        ...
        do ii=2,nn
           write(199,'(3F17.5)')x_L+ii*dh,c_f(ii),phi_f(x_L + ii*dh,d,0)
        enddo
        close(199)
