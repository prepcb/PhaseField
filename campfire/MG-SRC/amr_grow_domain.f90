!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! Domain growing routines
!!!! * Called from time-stepping loop if grow=1 
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! amr_grow_domain
!!!! * Main growing loop 
!!!! * Tests coarse grid for refinement at far field boundaries
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! test_other_surr_faces
!!!! * Tests other surrounding blocks 
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! set_other_surr_faces_3d
!!!! * Sets new block information on connecting blocks in 3-d
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! set_other_surr_faces_2d
!!!! * Sets new block information on connecting blocks in 2-d
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! process_other_surr_face
!!!! * Process new block info from other connected processors
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! face_growing_ploop_end
!!!! * Loop terminating routine for processors other than the one testing 
!!!!   for new blocks
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
...
!!!! 
!!!! debug_block
!!!! * Debug call to output connections of coarsest grid blocks
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!! 
!!!! graphviz_output
!!!! * Generate output data on coarsest grid to feed to GraphViz
!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
subroutine amr_grow_domain(pid, noprocs, step)
  ...
  integer :: i, j, k, p, fc, bc, tmpI, sending_len
...
!  cnt(2) = block_starts(2)-1
  ...
  do lb = 1, lnblocks
     if (lrefine(lb).eq.mg_min_lvl) cnt(2) = cnt(2) + 1
  end do
...
!print *,'allocate', cnt(2)
     ...
     cnt(2) = 0
     do lb = 1, lnblocks
        if (lrefine(lb).eq.mg_min_lvl) then
           cnt(2) = cnt(2) + 1
!print *,'used[1]', cnt(2)
           bklist(cnt(2)) = lb
        endif
...
!  if (Gcnt(1).eq.Gcnt(2)) then ! i.e. are there all blocks parents, hence necessitating domain growing 
  if (Gcnt(1).gt.0) then ! i.e. are there blocks parent with far-field boundaries, hence necessitating domain growing 
     ...
     if (pid.eq.0.and.verbose.ge.0)  write (6,'(A,A)') 27, "[31;1;44m"
     if (pid.eq.0.and.verbose.ge.1)  print *,'Coarse grid blocks grown - growing domain'
...
!     if (pid.eq.0) print*,'Now for rearrangement...'
     ...
     if (verbose.ge.2) call debug_block(pid)
     if (verbose.ge.4) call graphviz_output(pid, noprocs, step)
     call MPI_Barrier(MPI_COMM_WORLD, ierr)
     if (pid.eq.0.and.verbose.ge.0) write (6,'(A,A)') 27, "[0m"
...
subroutine get_neigh_dirs(nghbdry, ix, iy, iz, opp, sending_len)
  use paramesh_dimensions
  implicit none
  integer, intent(in) :: nghbdry
  integer, intent(out) ::  opp, sending_len
...
subroutine graphviz_output(pid, noprocs, step)
  use multigrid_parameters ! for mg_min_lvl
  ...
  do lb=1, lnblocks
     if (lrefine(lb).eq.mg_min_lvl) i = i+1
  end do
