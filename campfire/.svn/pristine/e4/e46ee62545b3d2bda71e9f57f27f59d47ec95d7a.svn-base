...
double precision :: nu_Pb=54730.,nu_Sn=61505.,psi,dpsi_dphi(3,3,3),f_mix,ff(3),c_,alpha=0.25
...
psi = 1d0 + 4d0*(f_phi(phi,1,0,0)*f_phi(phi,2,0,0)+f_phi(phi,2,0,0)*f_phi(phi,3,0,0)&
     +f_phi(phi,3,0,0)*f_phi(phi,1,0,0))*alpha

dpsi_dphi(lp,1,2)= 4d0*(f_phi(phi,1,lp,0)*f_phi(phi,2,0,0)+f_phi(phi,1,0,0)*f_phi(phi,2,lp,0))*alpha
dpsi_dphi(lp,2,3)= 4d0*(f_phi(phi,2,lp,0)*f_phi(phi,3,0,0)+f_phi(phi,2,0,0)*f_phi(phi,3,lp,0))*alpha
dpsi_dphi(lp,3,1)= 4d0*(f_phi(phi,3,lp,0)*f_phi(phi,1,0,0)+f_phi(phi,3,0,0)*f_phi(phi,1,lp,0))*alpha
...
! the following differentiates with respect to phi(lp)
    ...
    do i=1,N_phases
       ...
       FreeEnergy = FreeEnergy +ff(i)*f_phi(phi,i,lp,0) !a bit clumsy but ff(i) needs to be full component Free Energy
    ...
    FreeEnergy = FreeEnergy + dpsi_dphi(lp,3,1)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)
    FreeEnergy = FreeEnergy + dpsi_dphi(lp,1,2)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)
    FreeEnergy = FreeEnergy + dpsi_dphi(lp,2,3)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut)
    
    FreeEnergy = FreeEnergy/psi

    FreeEnergy = FreeEnergy -(ff(1)*f_phi(phi,1,0,0)+ff(2)*f_phi(phi,2,0,0)+ff(3)*f_phi(phi,3,0,0)&
     + alpha*4d0*f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)&
     + alpha*4d0*f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)&
     + alpha*4d0*f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut))&
     /psi**2*(dpsi_dphi(lp,3,1)+dpsi_dphi(lp,1,2)+dpsi_dphi(lp,2,3))
    return
...
! lamda is already non-dimensional
 FreeEnergy = FreeEnergy&
      + alpha*4d0*f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*lamda(3,1)&
      + alpha*4d0*f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*lamda(1,2)&
      + alpha*4d0*f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*lamda(2,3)
 FreeEnergy = FreeEnergy/psi
