...
implicit none
double precision, intent (in)::LapPhi(3),dx,vbles(9,3,3)
...
implicit none
double precision, intent (in):: dx,c,T,phi(3),c_dot,phi_dot(3),vbles(9,3,3)
...
!!
  ...
  TemperatureRHS = FreeEnergy(c,T,phi,c_dot,phi_dot,N_phases+1,vbles) !J K/mol
  ...
  if(heat_c_term)then
     do ii=1,3
        do jj=1,3
           ...
           enddo
           dFdc(ii,jj)=(1.+T_)*(log(c_)-log(1.-c_))+FreeEnergy(c_,T_,Phi_,c_dot,phi_dot,N_phases+2,vbles)
        enddo
     enddo
     D_heat_c = 0.
     do ii=1,N_phases
...
use solution_parameters
implicit none
double precision, intent (in):: dx,vbles(9,3,3)
...
double precision :: lamda,c1,c2,c3,spare!!
...
if(lp.le.N_phases)then 
   do ip=1,N_phases
      if(ip.ne.lp) potential = potential + eps22(lp,ip)*2d0*phi(lp)*phi(ip)**2 
   enddo
endif
return
...
double precision, intent(in)::dx,c,T,c_dot,phi_dot(3),Phi(3),vbles(9,3,3)
integer, intent(in):: lp
...
integer ip,n
double precision :: nu_Pb=54730.,nu_Sn=61505.,molpervol
double precision :: c1,c2,c3,lamda,Tk,T_k,f_phi
double precision :: ff,g1,g2,g3,phi_(3),R=8.31,T_eut=456.14,aa,bb
...
MolPerVol= nu_Pb*(1.-c)+nu_Sn*c
 ...
 do ip=1,3
    ff = ff + f_phi(phi,ip,lp,0)*(FreeEnergy(vbles(5+ip,2,2),T,phi,0d0,0d0,ip+11,vbles))
    ff = ff - f_phi(phi,ip,lp,0)*vbles(5+ip,2,2)*vbles(9,2,2)*R*T_eut
 enddo
...
! w_a(1,1)=7e7 ! see notes = 0.07 / delta = 0.07/1e-9 
 ...
 phaseRHS=phaseRHS + M_tilde*(-ff/lambda/W_a(1,1))
...
implicit none
!integer, intent(in)::df
double precision, intent(in)::dx,Phi(3),c,T,c_dot,phi_dot(3),vbles(9,3,3)
...
double precision vbles(9,3,3),unk_(30),c1,c2,c3,lamda
logical Not_number
integer ip,n,ii,jj,kk
  ...
  do ii=1,3
     do jj=1,3
        ...
        enddo
        do ip=6,9
           vbles(ip,ii,jj)=unk(total_vars*nunkvbles+ip-total_vars,i+ii-2,j+jj-2,k,lb)
        enddo
     enddo
  enddo
...
!  enddo
  do ip=6,9
     unk(total_vars*nunkvbles+ip-5,i,j,k,lb)=vbles(ip,2,2)
  enddo
...
double precision, intent(out):: Fi(5)
double precision, intent(inout):: vbles(9,3,3)
double precision :: c,T,phi(3),c_dot,phi_dot(3)
double precision phi_rhs(3),dphi_rhs(3),rfactor,rf4,rf5,rf6,S
double precision TemperatureRHS,SoluteRHS,T_k,phi_
double precision :: c1,c2,c3,lamda
logical Not_number
integer ip,n,ii,jj,kk
...
!lambda is df/dc
  ...
  Fi(N_phases+2) = SoluteRHS(vbles,dx)
...
double precision function FreeEnergy(c,T,phi,c_dot,phi_dot,lp,vbles)
use solution_parameters
implicit none

double precision ta,tb,L_a,L_b,R,mola,molb,rhoa(3),rhob(3)
!double precision,dimension (3,8)::hsa,hsb,hsrk
double precision, intent (in):: T,c,phi(3),c_dot,phi_dot(3),vbles(9,3,3)
integer, intent(in)::lp
...
elseif(lp.eq.N_phases+1)then !Temperature Keep in J/mol
   ...
   FreeEnergy = 0.
   do i= 1,N_phases
      call MixingVector(RK,vbles(5+i,2,2),0)
      ha=0.
      hb=0.
      hr=0.
      do j = 1,8
      ...
      do j=1,N_phases
         ...
         FreeEnergy = FreeEnergy + Tk*(vbles(5+i,2,2)*hb+(1.-vbles(5+i,2,2))*ha+hr)*f_phi(phi,i,j,0)*phi_dot(j)
      enddo
   enddo
...
!!!!!!!!!new bit
   ...
   do i= 1,N_phases
      call MixingVector(RK,vbles(5+i,2,2),1)
      ha=0.
      hb=0.
      hr=0.
      do j = 1,8
      ...
      do j=1,N_phases
         ! - c_i * df/dc_i * phi_dot
         FreeEnergy = FreeEnergy - Tk*(hb-ha+hr)*vbles(5+i,2,2)*f_phi(phi,i,j,0)*phi_dot(j)
      enddo
   enddo
...
!!!!!!!!!!new bit
   ...
   do i= 1,N_phases
      ...
      enddo
      do j=1,N_phases
         FreeEnergy = FreeEnergy - ((vbles(5+i,2,2)*hb+(1.-vbles(5+i,2,2))*ha+hr)-R*T_eut*vbles(5+i,2,2)*vbles(9,2,2))*f_phi(phi,i,j,0) *phi_dot(j)
      enddo
   enddo
   if(c_dot_term)then !there is a c_dot term see PRL small for large Le
      ...
      do i= 1,N_phases
         call MixingVector(RK,vbles(5+i,2,2),1)
         RE = R*EntropyMixing(vbles(5+i,2,2),1)
         ha=0.
         hb=0.
         hr=0.
         do j = 1,8
   ...
   do i= 1,N_phases
      ...
      do j = 1,8
         ...
         hb = hb + hsb(i,j)*V(j)
      enddo
      Cp = Cp - (vbles(5+i,2,2)*hb+(1.-vbles(5+i,2,2))*ha)*f_phi(phi,i,0,0)
   enddo
   Cp = Cp*Tk
   FreeEnergy = FreeEnergy/Cp
   
elseif(lp.gt.N_phases+2)then !the ci's
   if(lp.le.8)then
      i=lp-5
      FreeEnergy = 0.!debug                                                                                                                                                                                                                       
      call GibbsVector(V,Tk,0)
      ...
      ! non-dimensionalise Free energy                                                                                                                                                                                                            
      FreeEnergy = FreeEnergy/(R*T_eut) + Tk/T_eut*EntropyMixing(c,1)
      
   elseif(lp.le.11)then
      i=lp-8
      FreeEnergy = 0.!debug                                                                                                                                                                                                                       
      call GibbsVector(V,Tk,0)
      ...
      ! non-dimensionalise Free energy                                                                                                                                                                                                            
      FreeEnergy = FreeEnergy/(R*T_eut)+ Tk/T_eut*EntropyMixing(c,2)
   elseif(lp.le.14)then
      ...
      FreeEnergy=0.
         ...
         FreeEnergy = (c*hb+(1.-c)*ha+hr) + R*Tk*EntropyMixing(c,0)
