...
subroutine phase_field_check(pid, inumber)
  ...
  double precision :: new_loc, new_rad, tempr,temp2,temp3,temp4,temp_max,temp_min,heat(4)
...
!     if(.not.has_children(lb).and.on_axisx) then
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
              temp_max=max(unk(1+nunkvbles,i,j,1,lb),temp_max)
...
!  call MPI_REDUCE(new_rad,tempr,1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  ...
  call MPI_REDUCE(heat_size(1),heat(1),1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(heat_size(2),heat(2),1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(heat_size(3),heat(3),1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  call MPI_REDUCE(heat_size(4),heat(4),1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)


  heat_size=0d0 !its work is done. Zero for next time step
...
!  call MPI_REDUCE(dist,temp,1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  ...
  if(pid.eq.0)write(*,'(4e12.5)')heat  
  if(pid.eq.0)print *,"Tip position",temp2
  if(pid.eq.0)print *,"dt",dt
  if(pid.eq.0)write(126,'(7e14.5)') time, temp2,temp3,heat
...
subroutine pf_output_2d(pid, fnum)
  ...
  double precision ::c,T,phi,c_dot,phi_dot(3),rfactor,rf4,rf5,rf6
      ...
      dx = bsize(1,lb)/real(nxb)
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
             ...
             T=unk(1+1*nunkvbles,i,j,1,lb)
             c=unk(1+2*nunkvbles,i,j,1,lb)
