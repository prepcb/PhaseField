...
subroutine app_mg_smooth(pid,level)
  ...
  integer :: ii,jj
  logical Not_number
  double precision:: new_soln(il_bnd:iu_bnd,jl_bnd:ju_bnd,kl_bnd:ku_bnd,3),phi
  iopt=1
        ...
        do lb=block_starts(level), block_starts(level+1)-1
           ...
           do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d
              do i=il_bnd+nguard,iu_bnd-nguard
                 phi = max(0d0,new_soln(i,j,1,1))
                 new_soln(i,j,1,1)=min(1d0,phi)
                 new_soln(i,j,1,3) = min(1d0-1d-16,new_soln(i,j,1,3))
...
subroutine pf_mg_smooth_var(lb,level,new_soln,pid)
  ...
  double precision :: dx,Fi(12),D_Fi(12,12)=0d0,Wk(12)=0d0
  integer:: ifid
  logical :: mode_1_log
  logical Not_number
  ...
  do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d-1,2 
     do i=il_bnd+nguard,iu_bnd-nguard-1,2
        ...
        do ii=1,3
           do aa=1,4
              new_soln(i+rrarray(aa,1),j+rrarray(aa,2),k,ii)=unk(1+(ii-1)*nunkvbles,i+rrarray(aa,1),j+rrarray(aa,2),k,lb)-weight*Wk(ii+(aa-1)*3)
           enddo
        enddo
     end do
  end do
...
subroutine app_mg_get_defect_var(lb, max_defect, rms, npts)
  ...
  integer :: i,j,k,ii,n
  double precision :: dx,Fi(3)
  double precision :: rfactor, rf1, rf2, rf3
  double precision :: wt(3)=(/1e0,1e0,1e0/)
...
double precision, intent(inout) :: Fi(12),d_Fi(12,12)
double precision :: vblesd(3,3,3),hh=1d-10
integer ip,n,ii,jj,aa
double precision :: FiM(3,4),FidM(3,4), unkM(18,4)
double precision :: vblesM(3,3,3,4)
...
! the next  line rrblock(j,3)  refers to position in 2 x 2 elements, e.g. rrblock(1,3)=3 is (i,j+1)
...
double precision,intent(inout) :: AA(12,12),BB(12)
double precision,intent(out) :: X(12)
double precision ::  A(12,12),B(12)
