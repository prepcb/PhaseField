...
!              new_soln(i,j,k,1) = max(0d0,min(1d0,new_soln(i,j,k,1)))
              do current_var=1,N_unk
                 if(.not.Not_number(new_soln(i,j,k,current_var)))then
                    unk(1+(current_var-1)*nunkvbles,i,j,k,lb)=new_soln(i,j,k,current_var)
                 else
                    write(*,*)i,j,k,lb,current_var,level
        ...
        do i=il_bnd+nguard,iu_bnd-nguard
         if(current_var.eq.1)then
           ...
           do ii=1,N_unk
            ...
            if(max_defect.gt.0.9d0)then
               write(*,*)"defect",max_defect,lb
               write(*,*)"Fi"
               write(*,*)Fi
               write(*,*)"unk"
               write(*,*) unk(1,i,j,k,lb),unk(1+nunkvbles,i,j,k,lb),unk(1+2*nunkvbles,i,j,k,lb)
               write(*,*)"i,j,k,lb"
               write(*,*)i,j,k,lb
               stop
            endif
...
double precision :: FiM(N_unk,8),FidM(N_unk,8), unkM(M_unk,8)
double precision :: vblesM(N_unk+1,3,3,3,8),c,rfactor,rf4,rf5,rf6,MolPerVol
...
!  endif
  ...
  do aa=1,8
     do ii=1,M_unk
        unkM(ii,aa)= unk(ii,i+rrarray3d(aa,1),j+rrarray3d(aa,2),k+rrarray3d(aa,3),lb)
     enddo
enddo
...
!set up correct sign of "h" for numerical differentiation
  ...
  hh=abs(hh)
  if(ifid.eq.N_phases+1.and.thermal.eq..true.)then
     continue
