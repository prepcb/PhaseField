...
subroutine pf_mg_smooth_var(lb,level,new_soln,pid)
  ...
  double precision :: dx,Fi(80)=0d0,D_Fi(80,80)=0d0,Wk(80)=0d0
  integer:: ifid
  logical :: mode_1_log
  logical Not_number
  ...
  do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d-3,4 
     do i=il_bnd+nguard,iu_bnd-nguard-3,4
        ...
        do ii=1,5
           do aa=1,16
              new_soln(i+rrrarray(aa,1),j+rrrarray(aa,2),k,ii)=unk(1+(ii-1)*nunkvbles,i+rrrarray(aa,1),j+rrrarray(aa,2),k,lb)-weight*Wk(ii+(aa-1)*5)
           enddo
        enddo
     end do
  end do
...
double precision, intent(inout) :: Fi(80),d_Fi(80,80)
double precision :: unk_(30),vblesd(5,3,3),hh=1d-10
integer ip,n,ii,jj,aa
double precision :: FiM(5,16),FidM(5,16), unkM(30,16)
double precision :: vblesM(5,3,3,16)=0d0
logical :: Jacobi_cross=.true.
  ...
  do ii=1,3
     do jj=1,3
        do ip=1,5
           do aa = 1,16
              vblesM(ip,ii,jj,aa)=unk(1+(ip-1)*nunkvbles,i+ii-2+rrrarray(aa,1),j+jj-2+rrrarray(aa,2),1,lb)
           enddo
        enddo
     enddo
  enddo
...
! boundary condition. If Jacobi geaometry changes these lines must change too  
  if(neigh(1,2,lb).eq.bc_cond_sym.and. i.eq.8)then
     ...
     do jj=1,3
        ip=4
        vblesM(ip,ii,jj,6)=delta  ! 
        vblesM(ip,ii,jj,8)=delta  !
        vblesM(ip,ii,jj,14)=delta  ! 
        vblesM(ip,ii,jj,16)=delta  !
     enddo
  endif
  ...
  do aa=1,16
     do ii=1,30
        unkM(ii,aa)= unk(ii,i+rrrarray(aa,1),j+rrrarray(aa,2),1,lb)
     enddo
  enddo
...
! thus addressing the correct element in the 3 x 3 stencil, (2,2) being the central node.
  ...
  do jj=1,64
     ...
     vblesd=vblesM(:,:,:,rrrblock(jj,3))

     vblesd(ifid,rrrblock(jj,4),rrrblock(jj,5))=vblesM(ifid,rrrblock(jj,4),rrrblock(jj,5),rrrblock(jj,3))+hh


     call get_RHS_Fi(vblesd,dx,FidM(:,rrrblock(jj,3)),mode_1,nunkvbles,dt,dtold,lnblocks,unkM(:,rrrblock(jj,3)))

     do ip=1,5
        D_Fi(ip+rrrblock(jj,1),ifid+rrrblock(jj,2))=(fidM(ip,rrrblock(jj,3))-FiM(ip,rrrblock(jj,3)))/hh
        Fi(ip+rrrblock(jj,1)) = FiM(ip,rrrblock(jj,3))
     enddo
  enddo
...
double precision,intent(inout) :: AA(80,80),BB(80)
double precision,intent(out) :: X(80)
double precision ::  A(80,80),B(80)
