        ...
        do lb=block_starts(level), block_starts(level+1)-1
           ...
           do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d
              do i=il_bnd+nguard,iu_bnd-nguard
                 ...
                 if(Not_number(new_soln(i,j,1,3)))then
                    write(*,*)block_starts(level), block_starts(level+1)-1
                    write(*,*)"new soln=",new_soln(i,j,1,3)
                    write(*,*)unk(1,i,j,1,lb),unk(7,i,j,1,lb),unk(13,i,j,1,lb)
                    write(*,*)"position & block (",i,j,lb,")"
                    write(*,*)"dx=",dx
                    write(*,*)"x=(",bnd_box(1,1,lb)+(i-1.5)*dx,bnd_box(1,2,lb)+(j-1.5)*dx,")"
                    do ii=0,5
                    write(*,*)"--------------------------------------------------------"
                    write(*,*)unk(1+ii,i-1,j-1,1,lb),unk(7+ii,i,j-1,1,lb),unk(13+ii,i+1,j-1,1,lb)
                    write(*,*)unk(1+ii,i-1,j,1,lb),unk(7+ii,i,j,1,lb),unk(13+ii,i,j,1+1,lb)
                    write(*,*)unk(1+ii,i-1,j+1,1,lb),unk(7+ii,i,j+1,1,lb),unk(13+ii,i+1,j+1,1,lb)
                    write(*,*)"--------------------------------------------------------"
                    enddo
                    stop
...
double precision :: unk_(M_unk),vblesd(N_unk,3,3),hh=1d-10
integer ip,n,ii,jj,aa
double precision :: FiM(N_unk,4),FidM(N_unk,4), unkM(M_unk,4)
double precision :: vblesM(N_unk,3,3,4)
