...
!
!  Differentiation of a_func in forward (tangent) mode:
!   variations   of useful results: a_func
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
DOUBLE PRECISION FUNCTION A_FUNC_D(x, xd, y, yd, a_func)
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: x, y
  DOUBLE PRECISION, INTENT(IN) :: xd, yd
  DOUBLE PRECISION, SAVE :: epsilon_tilde=0d0
  DOUBLE PRECISION :: a_func
  a_func = 1d0 + epsilon_tilde
  IF (x + y .LT. 1d-11) THEN
    a_func_d = 0.D0
    RETURN
  ELSE
    a_func_d = (epsilon_tilde*(2*x*xd+2*y*yd)*(x+y)**2-epsilon_tilde*(x&
&     **2+y**2)*2*(x+y)*(xd+yd))/((x+y)**2)**2
    a_func = 1d0 + epsilon_tilde*(x**2+y**2)/(x+y)**2
  END IF
END FUNCTION A_FUNC_D
!        Generated by TAPENADE     (INRIA, Tropics team)
!  Tapenade 3.9 (r5096) - 24 Feb 2014 16:54
!
...
!integer, intent(in)::i,j,k,lb
  DOUBLE PRECISION, INTENT(IN) :: dx, vbles(6, 3, 3)
  DOUBLE PRECISION, INTENT(IN) :: vblesd(6, 3, 3)
  ...
  DOUBLE PRECISION, DIMENSION(3, 3), SAVE :: stencilyy
  DOUBLE PRECISION, DIMENSION(3, 3), SAVE :: stencilxx
  DOUBLE PRECISION, DIMENSION(3, 3), SAVE :: stencilx
  INTEGER :: ip, n, ii, jj
  DOUBLE PRECISION :: xx, yy, phi_x, phi_y, A_FUNC, a_
  DOUBLE PRECISION :: xxd, yyd, phi_xd, phi_yd, A_FUNC_D, a_d
  LOGICAL :: not_number
  LOGICAL, SAVE :: nine_point=.true.
  INTEGER, SAVE :: n_phases=3
  DOUBLE PRECISION, SAVE :: epsilon_tilde=0d0
  INTRINSIC RESHAPE
  REAL, DIMENSION(9) :: arg1
  REAL, DIMENSION(9) :: arg2
  arg1(:) = (/1., 4., 1., 4., -20., 4., 1., 4., 1./)
  stencil = RESHAPE(arg1(:), (/3, 3/))
  arg2(:) = (/0., 1., 0., 1., -4., 1., 0., 1., 0./)
  stencil5 = RESHAPE(arg2(:), (/3, 3/))
  arg1(:) = (/0., 1., 0., 0., -2., 0., 0., 1., 0./)
  stencilyy = RESHAPE(arg1(:), (/3, 3/))
  arg1(:) = (/0., 0., 0., 1., -2., 1., 0., 0., 0./)
  stencilxx = RESHAPE(arg1(:), (/3, 3/))
  arg1(:) = (/0., 0., 0., -.5, 0., .5, 0., 0., 0./)
  stencilx = RESHAPE(arg1(:), (/3, 3/))
  lapphi = 0.
  lapphid = 0.D0
  DO ip=1,n_phases
    IF (epsilon_tilde .GT. 1d-10) THEN
      phi_xd = 0.5*(vblesd(ip, 3, 2)-vblesd(ip, 1, 2))/dx
      phi_x = 0.5*(vbles(ip, 3, 2)-vbles(ip, 1, 2))/dx
      phi_yd = 0.5*(vblesd(ip, 2, 3)-vblesd(ip, 2, 1))/dx
      phi_y = 0.5*(vbles(ip, 2, 3)-vbles(ip, 2, 1))/dx
      xxd = 2*phi_x*phi_xd
      xx = phi_x**2
      yyd = 2*phi_y*phi_yd
      yy = phi_y**2
      a_d = A_FUNC_D(xx, xxd, yy, yyd, a_)
    ELSE
      xx = 0.
      yy = 0.
      a_ = 1.0
    ...
    IF (xx + yy .LT. 1d-8) THEN
      DO ii=1,3
        DO jj=1,3
          IF (nine_point) THEN
            lapphid(ip) = lapphid(ip) + stencil(ii, jj)*vblesd(ip, ii, &
&             jj)
            lapphi(ip) = lapphi(ip) + stencil(ii, jj)*vbles(ip, ii, jj)
          ELSE
            lapphid(ip) = lapphid(ip) + stencil5(ii, jj)*vblesd(ip, ii, &
&             jj)
      ...
      END DO
      IF (nine_point) THEN
        lapphid(ip) = ((a_d*a_+a_*a_d)*lapphi(ip)+a_**2*lapphid(ip))/(6.&
&         *dx*dx)
        lapphi(ip) = a_*a_*lapphi(ip)/(6.*dx*dx)
      ELSE
        lapphid(ip) = ((a_d*a_+a_*a_d)*lapphi(ip)+a_**2*lapphid(ip))/(dx&
&         *dx)
...
! x right
      phi_xd = (vblesd(ip, 3, 2)-vblesd(ip, 2, 2))/dx
      phi_x = (vbles(ip, 3, 2)-vbles(ip, 2, 2))/dx
      phi_yd = 0.25*(vblesd(ip, 2, 3)-vblesd(ip, 2, 1)+vblesd(ip, 3, 3)-&
&       vblesd(ip, 3, 1))/dx
      phi_y = 0.25*(vbles(ip, 2, 3)-vbles(ip, 2, 1)+vbles(ip, 3, 3)-&
&       vbles(ip, 3, 1))/dx
      xxd = 2*phi_x*phi_xd
      xx = phi_x**2
      yyd = 2*phi_y*phi_yd
      yy = phi_y**2
      a_d = A_FUNC_D(xx, xxd, yy, yyd, a_)
      lapphid(ip) = lapphid(ip) + ((a_d*phi_x+a_*phi_xd)*(epsilon_tilde*&
&       2.*yy*(xx-yy)/(xx+yy)**2+a_)+a_*phi_x*((epsilon_tilde*2.*(yyd*(&
&       xx-yy)+yy*(xxd-yyd))*(xx+yy)**2-epsilon_tilde*2.*yy*(xx-yy)*2*(&
&       xx+yy)*(xxd+yyd))/((xx+yy)**2)**2+a_d))/dx
      lapphi(ip) = lapphi(ip) + a_*(phi_x*(epsilon_tilde*2.*yy*(xx-yy)/(&
&       xx+yy)**2+a_))/dx
! x left
      phi_xd = (vblesd(ip, 1, 2)-vblesd(ip, 2, 2))/dx
      phi_x = (vbles(ip, 1, 2)-vbles(ip, 2, 2))/dx
      phi_yd = 0.25*(vblesd(ip, 2, 3)-vblesd(ip, 2, 1)+vblesd(ip, 1, 3)-&
&       vblesd(ip, 1, 1))/dx
      phi_y = 0.25*(vbles(ip, 2, 3)-vbles(ip, 2, 1)+vbles(ip, 1, 3)-&
&       vbles(ip, 1, 1))/dx
      xxd = 2*phi_x*phi_xd
      xx = phi_x**2
      yyd = 2*phi_y*phi_yd
...
&       xx+yy)**2+a_))/dx
! y up
      phi_yd = (vblesd(ip, 2, 3)-vblesd(ip, 2, 2))/dx
      phi_y = (vbles(ip, 2, 3)-vbles(ip, 2, 2))/dx
      phi_xd = 0.25*(vblesd(ip, 3, 2)-vblesd(ip, 1, 2)+vblesd(ip, 3, 3)-&
&       vblesd(ip, 1, 3))/dx
      phi_x = 0.25*(vbles(ip, 3, 2)-vbles(ip, 1, 2)+vbles(ip, 3, 3)-&
&       vbles(ip, 1, 3))/dx
      xxd = 2*phi_x*phi_xd
      xx = phi_x**2
      yyd = 2*phi_y*phi_yd
      yy = phi_y**2
      a_d = A_FUNC_D(xx, xxd, yy, yyd, a_)
      lapphid(ip) = lapphid(ip) + ((a_d*phi_y+a_*phi_yd)*(epsilon_tilde*&
&       2.*xx*(yy-xx)/(xx+yy)**2+a_)+a_*phi_y*((epsilon_tilde*2.*(xxd*(&
&       yy-xx)+xx*(yyd-xxd))*(xx+yy)**2-epsilon_tilde*2.*xx*(yy-xx)*2*(&
&       xx+yy)*(xxd+yyd))/((xx+yy)**2)**2+a_d))/dx
      lapphi(ip) = lapphi(ip) + a_*(phi_y*(epsilon_tilde*2.*xx*(yy-xx)/(&
&       xx+yy)**2+a_))/dx
! y down
      phi_yd = (vblesd(ip, 2, 1)-vblesd(ip, 2, 2))/dx
      phi_y = (vbles(ip, 2, 1)-vbles(ip, 2, 2))/dx
      phi_xd = 0.25*(vblesd(ip, 3, 2)-vblesd(ip, 1, 2)+vblesd(ip, 3, 1)-&
&       vblesd(ip, 1, 1))/dx
      phi_x = 0.25*(vbles(ip, 3, 2)-vbles(ip, 1, 2)+vbles(ip, 3, 1)-&
&       vbles(ip, 1, 1))/dx
      xxd = 2*phi_x*phi_xd
      xx = phi_x**2
      yyd = 2*phi_y*phi_yd
...
!    these terms to reduce grid anisotropy
      IF (nine_point) THEN
!    top right
        phi_xd = 0.5*(vblesd(ip, 3, 3)-vblesd(ip, 2, 3)+vblesd(ip, 3, 2)&
&         -vblesd(ip, 2, 2))/dx
        phi_x = 0.5*(vbles(ip, 3, 3)-vbles(ip, 2, 3)+vbles(ip, 3, 2)-&
&         vbles(ip, 2, 2))/dx
        phi_yd = 0.5*(vblesd(ip, 3, 3)-vblesd(ip, 3, 2)+vblesd(ip, 2, 3)&
&         -vblesd(ip, 2, 2))/dx
        phi_y = 0.5*(vbles(ip, 3, 3)-vbles(ip, 3, 2)+vbles(ip, 2, 3)-&
&         vbles(ip, 2, 2))/dx
        xxd = 2*phi_x*phi_xd
        xx = phi_x**2
        yyd = 2*phi_y*phi_yd
        yy = phi_y**2
        a_d = A_FUNC_D(xx, xxd, yy, yyd, a_)
        lapphid(ip) = lapphid(ip) + 0.5*((a_d*phi_x+a_*phi_xd)*(&
&         epsilon_tilde*2.*yy*(xx-yy)/(xx+yy)**2+a_)+a_*phi_x*((&
&         epsilon_tilde*2.*(yyd*(xx-yy)+yy*(xxd-yyd))*(xx+yy)**2-&
&         epsilon_tilde*2.*yy*(xx-yy)*2*(xx+yy)*(xxd+yyd))/((xx+yy)**2)&
&         **2+a_d))/dx
        lapphi(ip) = lapphi(ip) + 0.5*a_*(phi_x*(epsilon_tilde*2.*yy*(xx&
&         -yy)/(xx+yy)**2+a_))/dx
        lapphid(ip) = lapphid(ip) + 0.5*((a_d*phi_y+a_*phi_yd)*(&
&         epsilon_tilde*2.*xx*(yy-xx)/(xx+yy)**2+a_)+a_*phi_y*((&
&         epsilon_tilde*2.*(xxd*(yy-xx)+xx*(yyd-xxd))*(xx+yy)**2-&
&         epsilon_tilde*2.*xx*(yy-xx)*2*(xx+yy)*(xxd+yyd))/((xx+yy)**2)&
&         **2+a_d))/dx
        lapphi(ip) = lapphi(ip) + 0.5*a_*(phi_y*(epsilon_tilde*2.*xx*(yy&
&         -xx)/(xx+yy)**2+a_))/dx
!    top left
        phi_xd = 0.5*(vblesd(ip, 1, 3)-vblesd(ip, 2, 3)+vblesd(ip, 1, 2)&
&         -vblesd(ip, 2, 2))/dx
        phi_x = 0.5*(vbles(ip, 1, 3)-vbles(ip, 2, 3)+vbles(ip, 1, 2)-&
&         vbles(ip, 2, 2))/dx
        phi_yd = 0.5*(vblesd(ip, 1, 3)-vblesd(ip, 1, 2)+vblesd(ip, 2, 3)&
&         -vblesd(ip, 2, 2))/dx
        phi_y = 0.5*(vbles(ip, 1, 3)-vbles(ip, 1, 2)+vbles(ip, 2, 3)-&
&         vbles(ip, 2, 2))/dx
        xxd = 2*phi_x*phi_xd
        xx = phi_x**2
        yyd = 2*phi_y*phi_yd
...
&         -xx)/(xx+yy)**2+a_))/dx
!    bottom left
        phi_xd = 0.5*(vblesd(ip, 1, 1)-vblesd(ip, 2, 1)+vblesd(ip, 1, 2)&
&         -vblesd(ip, 2, 2))/dx
        phi_x = 0.5*(vbles(ip, 1, 1)-vbles(ip, 2, 1)+vbles(ip, 1, 2)-&
&         vbles(ip, 2, 2))/dx
        phi_yd = 0.5*(vblesd(ip, 1, 1)-vblesd(ip, 1, 2)+vblesd(ip, 2, 1)&
&         -vblesd(ip, 2, 2))/dx
        phi_y = 0.5*(vbles(ip, 1, 1)-vbles(ip, 1, 2)+vbles(ip, 2, 1)-&
&         vbles(ip, 2, 2))/dx
        xxd = 2*phi_x*phi_xd
        xx = phi_x**2
        yyd = 2*phi_y*phi_yd
...
&         -xx)/(xx+yy)**2+a_))/dx
!    bottom right
        phi_xd = 0.5*(vblesd(ip, 3, 1)-vblesd(ip, 2, 1)+vblesd(ip, 3, 2)&
&         -vblesd(ip, 2, 2))/dx
        phi_x = 0.5*(vbles(ip, 3, 1)-vbles(ip, 2, 1)+vbles(ip, 3, 2)-&
&         vbles(ip, 2, 2))/dx
        phi_yd = 0.5*(vblesd(ip, 3, 1)-vblesd(ip, 3, 2)+vblesd(ip, 2, 1)&
&         -vblesd(ip, 2, 2))/dx
        phi_y = 0.5*(vbles(ip, 3, 1)-vbles(ip, 3, 2)+vbles(ip, 2, 1)-&
&         vbles(ip, 2, 2))/dx
        xxd = 2*phi_x*phi_xd
        xx = phi_x**2
        yyd = 2*phi_y*phi_yd
...
!   variations   of useful results: cross_term
!   with respect to varying inputs: vbles phi_dot c_dot
DOUBLE PRECISION FUNCTION CROSS_TERM_D(dx, c_dot, c_dotd, phi_dot, &
& phi_dotd, vbles, vblesd, cross_term)
  USE SOLUTION_PARAMETERS
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: dx, c_dot, phi_dot(3), vbles(6, 3, 3)
  DOUBLE PRECISION, INTENT(IN) :: c_dotd, phi_dotd(3), vblesd(6, 3, 3)
  DOUBLE PRECISION :: FREEENERGY, a, d, d_c, d_cc, soluterhs, F_C, F_CC&
& , c_c, c, t, phi(3), POTENTIAL
  DOUBLE PRECISION :: FREEENERGY_D, dd, d_cd, d_ccd, soluterhsd, F_C_D, &
& F_CC_D, cd, td, phid(3), POTENTIAL_D
!Jackson Hunt
  DOUBLE PRECISION, DIMENSION(3), SAVE :: d1=(/6.7e-10, 6.7e-12, 6.7e-12&
&   /)
  DOUBLE PRECISION, SAVE :: beta=0d0, PHASERHS_C_CD
  DOUBLE PRECISION :: PHASERHS_C_D
  DOUBLE PRECISION, SAVE :: beta_=0d0
  INTEGER :: ih, jv, ip
...
!! Right
  ...
  d_ccd = 0.D0
  DO ip=1,n_phases
    dd = dd + 0.5*d1(ip)*(vblesd(4, ih, jv)*vbles(ip, ih, jv)+(1.+vbles(&
&     4, ih, jv))*vblesd(ip, ih, jv)+vblesd(4, 2, 2)*vbles(ip, 2, 2)+(1.&
...
&     jv)*result1d+result2*vblesd(ip, 2, 2))
...
&   phi_dot, phi_dotd, n_phases + 2, result1)
  ...
  result3d = PHASERHS_C_D(vbles, vblesd, dx, 1, phi, phid, c, cd, t, td&
&   , c_dot, c_dotd, phi_dot, phi_dotd, result3)
  soluterhsd = d_cd*(result1+result2+beta*0.34*result3) + d_c*(result1d+&
&   result2d+beta*0.34*result3d) + dd*c + d*cd + d_ccd*t + d_cc*td
  soluterhs = soluterhs + d_c*(result1+result2+beta*0.34*result3) + d*c &
&   + d_cc*t
!  centre
  cd = vblesd(5, 2, 2)
  c = vbles(5, 2, 2)
...
&   phi_dot, phi_dotd, n_phases + 2, result1)
  ...
  result3d = PHASERHS_C_D(vbles, vblesd, dx, 1, phi, phid, c, cd, t, td&
&   , c_dot, c_dotd, phi_dot, phi_dotd, result3)
  soluterhsd = soluterhsd - d_cd*(result1+result2+beta*0.34*result3) - &
&   d_c*(result1d+result2d+beta*0.34*result3d) - dd*c - d*cd - d_ccd*t -&
&   d_cc*td
  soluterhs = soluterhs - d_c*(result1+result2+beta*0.34*result3) - d*c &
&   - d_cc*t
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
&   d_cc*td
  soluterhs = soluterhs - d_c*(result1+result2+beta*0.34*result3) - d*c &
&   - d_cc*t
  cross_term_d = beta_*soluterhsd/2.83d-10/(dx*dx)
...
& c_dotd, phi_dot, phi_dotd, lp, freeenergy)
  ...
  DOUBLE PRECISION, DIMENSION(3, 8) :: hsad, hsbd, hsrkd
...
!   with respect to varying inputs: vbles
!   RW status of diff variables: vbles:in-out fi:out
SUBROUTINE GET_RHS_FI_D(vbles, vblesd, dx, fi, fid, mode_1,  &
& nunkvbles, dt, dtold, lnblocks, unk_)
  use solution_parameters
  IMPLICIT NONE

  INTEGER, INTENT(IN) ::  mode_1, nunkvbles, lnblocks
  DOUBLE PRECISION, INTENT(IN) :: dx, dt, dtold, unk_(36)
  DOUBLE PRECISION, INTENT(OUT) :: fi(6)
  DOUBLE PRECISION, INTENT(OUT) :: fid(6)
  ...
  DOUBLE PRECISION :: SOLUTERHS_D, T_K_D,soluterhs
  DOUBLE PRECISION :: vbles(6, 3, 3)
  DOUBLE PRECISION :: vblesd(6, 3, 3)
  LOGICAL :: not_number
...
!   phi_dot(ip) = (rf4*unk(5+n,i,j,k,lb)-rf5*unk(4+n,i,j,k,lb)+rf6*unk (1+n, i, j, k, lb))/dt
      phi_dotd(ip) = rf6*vblesd(ip, 2, 2)/dt
...
!   S=unk (1, i, j, k, lb)+unk (1+nunkvbles, i, j, k, lb)+unk (1+2*nunkvbles, i, j, k, lb)
  ...
  DO ip=1,n_phases
    ...
    IF (1d0 .GT. phi(ip)/(phi(1)+phi(2)+phi(3))) THEN
      phid(ip) = (phid(ip)*(phi(1)+phi(2)+phi(3))-phi(ip)*(phid(1)+phid(&
&       2)+phid(3)))/(phi(1)+phi(2)+phi(3))**2
      phi(ip) = phi(ip)/(phi(1)+phi(2)+phi(3))
    ELSE
      phid(ip) = 0.D0
      phi(ip) = 1d0
    END IF
!     unk (1+n, i, j, k, lb)=phi(ip)
    vblesd(ip, 2, 2) = phid(ip)
    vbles(ip, 2, 2) = phi(ip)
  END DO
...
&   phi_dot, phi_dotd, c, cd, fi(n_phases+2))
  Fi(N_phases+3) = -lambda_c*(vbles(5,2,1)+vbles(5,1,2)+vbles(5,2,3)+vbles(5,3,2)-4d0*vbles(5,2,2))/dx**2
  Fid(N_phases+3) = -lambda_c*(vblesd(5,2,1)+vblesd(5,1,2)+vblesd(5,2,3)+vblesd(5,3,2)-4d0*vblesd(5,2,2))/dx**2
...
use solution_parameters
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: lapphi(3), dx, vbles(6, 3, 3)
  DOUBLE PRECISION, INTENT(IN) :: lapphid(3), vblesd(6, 3, 3)
  INTEGER, INTENT(IN) :: lp
  DOUBLE PRECISION :: phi_x(3), phi_y(3), phi(3), phi_xx(3), phi_yy(3)
  ...
  DO ii=1,n_phases
    IF (ii .NE. lp) THEN
      gradientenergy_d = gradientenergy_d + eps22(lp, ii)*(2.*((phid(lp)&
&       *phi_x(ii)+phi(lp)*phi_xd(ii)-phid(ii)*phi_x(lp)-phi(ii)*phi_xd(&
&       lp))*phi_x(ii)+(phi(lp)*phi_x(ii)-phi(ii)*phi_x(lp))*phi_xd(ii)+&
&       (phid(lp)*phi_y(ii)+phi(lp)*phi_yd(ii)-phid(ii)*phi_y(lp)-phi(ii&
...
&       lp)*lapphi(ii))*phid(ii))
      gradientenergy = gradientenergy + eps22(lp, ii)*(2.*((phi(lp)*&
&       phi_x(ii)-phi(ii)*phi_x(lp))*phi_x(ii)+(phi(lp)*phi_y(ii)-phi(ii&
&       )*phi_y(lp))*phi_y(ii))-(phi(ii)*lapphi(lp)-phi(lp)*lapphi(ii))*&
&       phi(ii))
    END IF
  END DO
  gradientenergy_d = -gradientenergy_d
...
DOUBLE PRECISION FUNCTION PHASERHS_C_D(vbles, vblesd, dx, lp, phi, phid&
& , c, cd, t, td, c_dot, c_dotd, phi_dot, phi_dotd, phaserhs_c)
  USE SOLUTION_PARAMETERS
  IMPLICIT NONE
!integer, intent(in)::i,j,k,lb
...
&   phi_dot, phi_dotd, lp, result3)
  phaserhs_c_d = -(lambda**2*result1d-scale_factor**2*result2d-&
&   scale_factor**2*lambda*result3d/w_a(1, 1))
  phaserhs_c = -(lambda**2*result1-result2*scale_factor**2-scale_factor&
&   **2*lambda*result3/w_a(1, 1))
...
!m^3/(J s) mobility
  ...
  DOUBLE PRECISION, SAVE :: delta2=4e-18
  DOUBLE PRECISION :: CROSS_TERM_CD
...
&   scale_factor**2/lambda*result3/w_a(1, 1))
  IF (lp .EQ. 1) THEN
    result1d = CROSS_TERM_D(dx, c_dot, c_dotd, phi_dot, phi_dotd, vbles&
&     , vblesd, result1)
...
use solution_parameters
 ...
 do ip=1,N_phases
     ...
     potential_d = potential_d + eps22(lp,ip)*2d0*phid(lp)*phi(ip)**2 
     potential_d = potential_d + eps22(lp,ip)*4d0*phi(lp)*phi(ip)*phid(ip) 
 enddo
...
& vblesd(6, 3, 3)
  ...
  DOUBLE PRECISION, DIMENSION(3, 3, 3), SAVE :: d_projector=0.
  DOUBLE PRECISION, DIMENSION(3, 3), SAVE :: l2
  INTRINSIC RESHAPE
  REAL, DIMENSION(9) :: arg1
  arg1(:) = (/1., 0., 0., 0., 1., 0., 0., 0., 1./)
  i2 = RESHAPE(arg1(:), (/3, 3/))
  arg1(:) = (/2., -1., -1., -1., 2., -1., -1., -1., 2./)
  l2 = RESHAPE(arg1(:), (/3, 3/))
...
! maple generated A
    projectord = 0.D0
    projectord(1, 1) = phid(1) + ((phid(2)*phi(1)**2+phi(2)*2*phi(1)*&
&     phid(1))*(1-phi(2))**2+phi(2)*phi(1)**2*2*(1-phi(2))*phid(2))/((1-&
&     phi(2))**2)**2 + ((phid(3)*phi(1)**2+phi(3)*2*phi(1)*phid(1))*(1-&
&     phi(3))**2+phi(3)*phi(1)**2*2*(1-phi(3))*phid(3))/((1-phi(3))**2)&
&     **2
    projector(1, 1) = phi(1) + phi(2)*phi(1)**2/(1-phi(2))**2 + phi(3)*&
&     phi(1)**2/(1-phi(3))**2
    projectord(1, 2) = (((phid(3)*phi(1)+phi(3)*phid(1))*phi(2)+phi(3)*&
&     phi(1)*phid(2))*(1-phi(3))**2+phi(3)*phi(1)*phi(2)*2*(1-phi(3))*&
&     phid(3))/((1-phi(3))**2)**2 - phi(1)*phid(2)/(1-phi(1)) - ((phid(2&
&     )*phi(1)+phi(2)*phid(1))*(1-phi(2))+phi(2)*phi(1)*phid(2))/(1-phi(&
&     2))**2 - (phid(1)*(1-phi(1))+phi(1)*phid(1))*phi(2)/(1-phi(1))**2
    projector(1, 2) = -(phi(1)/(1-phi(1))*phi(2)) - phi(2)*phi(1)/(1-phi&
&     (2)) + phi(3)*phi(1)*phi(2)/(1-phi(3))**2
    projectord(1, 3) = (((phid(2)*phi(1)+phi(2)*phid(1))*phi(3)+phi(2)*&
&     phi(1)*phid(3))*(1-phi(2))**2+phi(2)*phi(1)*phi(3)*2*(1-phi(2))*&
&     phid(2))/((1-phi(2))**2)**2 - phi(1)*phid(3)/(1-phi(1)) - (phid(1)&
&     *(1-phi(1))+phi(1)*phid(1))*phi(3)/(1-phi(1))**2 - ((phid(3)*phi(1&
&     )+phi(3)*phid(1))*(1-phi(3))+phi(3)*phi(1)*phid(3))/(1-phi(3))**2
    projector(1, 3) = -(phi(1)/(1-phi(1))*phi(3)) + phi(2)*phi(1)*phi(3)&
&     /(1-phi(2))**2 - phi(3)*phi(1)/(1-phi(3))
    projectord(2, 1) = (((phid(3)*phi(1)+phi(3)*phid(1))*phi(2)+phi(3)*&
&     phi(1)*phid(2))*(1-phi(3))**2+phi(3)*phi(1)*phi(2)*2*(1-phi(3))*&
&     phid(3))/((1-phi(3))**2)**2 - phi(1)*phid(2)/(1-phi(1)) - ((phid(2&
&     )*phi(1)+phi(2)*phid(1))*(1-phi(2))+phi(2)*phi(1)*phid(2))/(1-phi(&
&     2))**2 - (phid(1)*(1-phi(1))+phi(1)*phid(1))*phi(2)/(1-phi(1))**2
    projector(2, 1) = -(phi(1)/(1-phi(1))*phi(2)) - phi(2)*phi(1)/(1-phi&
&     (2)) + phi(3)*phi(1)*phi(2)/(1-phi(3))**2
    projectord(2, 2) = ((phid(1)*phi(2)**2+phi(1)*2*phi(2)*phid(2))*(1-&
&     phi(1))**2+phi(1)*phi(2)**2*2*(1-phi(1))*phid(1))/((1-phi(1))**2)&
&     **2 + phid(2) + ((phid(3)*phi(2)**2+phi(3)*2*phi(2)*phid(2))*(1-&
&     phi(3))**2+phi(3)*phi(2)**2*2*(1-phi(3))*phid(3))/((1-phi(3))**2)&
&     **2
    projector(2, 2) = phi(1)*phi(2)**2/(1-phi(1))**2 + phi(2) + phi(3)*&
&     phi(2)**2/(1-phi(3))**2
    projectord(2, 3) = (((phid(1)*phi(2)+phi(1)*phid(2))*phi(3)+phi(1)*&
&     phi(2)*phid(3))*(1-phi(1))**2+phi(1)*phi(2)*phi(3)*2*(1-phi(1))*&
&     phid(1))/((1-phi(1))**2)**2 - (phid(2)*(1-phi(2))+phi(2)*phid(2))*&
&     phi(3)/(1-phi(2))**2 - phi(2)*phid(3)/(1-phi(2)) - ((phid(3)*phi(2&
&     )+phi(3)*phid(2))*(1-phi(3))+phi(3)*phi(2)*phid(3))/(1-phi(3))**2
    projector(2, 3) = phi(1)*phi(2)*phi(3)/(1-phi(1))**2 - phi(2)/(1-phi&
&     (2))*phi(3) - phi(3)*phi(2)/(1-phi(3))
    projectord(3, 1) = (((phid(2)*phi(1)+phi(2)*phid(1))*phi(3)+phi(2)*&
&     phi(1)*phid(3))*(1-phi(2))**2+phi(2)*phi(1)*phi(3)*2*(1-phi(2))*&
&     phid(2))/((1-phi(2))**2)**2 - phi(1)*phid(3)/(1-phi(1)) - (phid(1)&
&     *(1-phi(1))+phi(1)*phid(1))*phi(3)/(1-phi(1))**2 - ((phid(3)*phi(1&
&     )+phi(3)*phid(1))*(1-phi(3))+phi(3)*phi(1)*phid(3))/(1-phi(3))**2
    projector(3, 1) = -(phi(1)/(1-phi(1))*phi(3)) + phi(2)*phi(1)*phi(3)&
&     /(1-phi(2))**2 - phi(3)*phi(1)/(1-phi(3))
    projectord(3, 2) = (((phid(1)*phi(2)+phi(1)*phid(2))*phi(3)+phi(1)*&
&     phi(2)*phid(3))*(1-phi(1))**2+phi(1)*phi(2)*phi(3)*2*(1-phi(1))*&
&     phid(1))/((1-phi(1))**2)**2 - (phid(2)*(1-phi(2))+phi(2)*phid(2))*&
&     phi(3)/(1-phi(2))**2 - phi(2)*phid(3)/(1-phi(2)) - ((phid(3)*phi(2&
&     )+phi(3)*phid(2))*(1-phi(3))+phi(3)*phi(2)*phid(3))/(1-phi(3))**2
    projector(3, 2) = phi(1)*phi(2)*phi(3)/(1-phi(1))**2 - phi(2)/(1-phi&
&     (2))*phi(3) - phi(3)*phi(2)/(1-phi(3))
    projectord(3, 3) = ((phid(1)*phi(3)**2+phi(1)*2*phi(3)*phid(3))*(1-&
&     phi(1))**2+phi(1)*phi(3)**2*2*(1-phi(1))*phid(1))/((1-phi(1))**2)&
&     **2 + ((phid(2)*phi(3)**2+phi(2)*2*phi(3)*phid(3))*(1-phi(2))**2+&
&     phi(2)*phi(3)**2*2*(1-phi(2))*phid(2))/((1-phi(2))**2)**2 + phid(3&
&     )
    projector(3, 3) = phi(1)*phi(3)**2/(1-phi(1))**2 + phi(2)*phi(3)**2/&
&     (1-phi(2))**2 + phi(3)
...
& phi_dot, phi_dotd, c_c, c_cd, soluterhs)
  IMPLICIT NONE
  INTEGER, SAVE :: n_phases=3
  DOUBLE PRECISION, INTENT(IN) :: dx, c_dot, c_c, vbles(6, 3, 3)
  DOUBLE PRECISION, INTENT(IN) :: c_dotd, c_cd, vblesd(6, 3, 3)
!integer, intent (in)::i,j,k,lb,df
  INTEGER :: ii, jj, ip, ih, jv
  DOUBLE PRECISION :: c, t, phi(3), FREEENERGY, d, phi_dot(3), phi_, &
& phi_tmp(3)
...
! (m^2/s)    characteristic diffusivity delta2/tau=M*eps2(1)
  ...
  DOUBLE PRECISION :: PHASERHS_C_CD
  DOUBLE PRECISION :: PHASERHS_C_D
  DOUBLE PRECISION, SAVE :: beta=0d0
  INTRINSIC RESHAPE
  REAL, DIMENSION(9) :: arg1
  DOUBLE PRECISION :: result1
  DOUBLE PRECISION :: result1d
...
&   phi_dot, phi_dotd, n_phases + 2, result1)
  ...
  result3d = PHASERHS_C_D(vbles, vblesd, dx, 1, phi, phid, c, cd, t, td&
&   , c_dot, c_dotd, phi_dot, phi_dotd, result3)
  soluterhs_d = d_cd*(result1+result2+beta*0.34*result3) + d_c*(result1d&
&   +result2d+beta*0.34*result3d) + dd*c + d*cd + d_ccd*t + d_cc*td
  soluterhs = soluterhs + d_c*(result1+result2+beta*0.34*result3) + d*c &
&   + d_cc*t
  soluterhs = soluteRHS +D_c*vbles(6,ih,jv)
  soluterhs_d = soluterhs_d +D_c*vblesd(6,ih,jv)+d_cd*vbles(6,ih,jv)
!  centre
  cd = vblesd(5, 2, 2)
  c = vbles(5, 2, 2)
  td = vblesd(4, 2, 2)
...
&   phi_dot, phi_dotd, n_phases + 2, result1)
  ...
  result3d = PHASERHS_C_D(vbles, vblesd, dx, 1, phi, phid, c, cd, t, td&
&   , c_dot, c_dotd, phi_dot, phi_dotd, result3)
  soluterhs_d = soluterhs_d - d_cd*(result1+result2+beta*0.34*result3) -&
&   d_c*(result1d+result2d+beta*0.34*result3d) - dd*c - d*cd - d_ccd*t -&
&   d_cc*td
  soluterhs = soluterhs - d_c*(result1+result2+beta*0.34*result3) - d*c &
&   - d_cc*t
  soluterhs = soluteRHS -D_c*vbles(6,2,2)
  soluterhs_d = soluterhs_d -D_c*vblesd(6,2,2)-d_cd*vbles(6,2,2)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!Left
  ih = 1
  jv = 2
