...
! include files 
  ...
  double precision :: steepness=5.0,snap21,spike,eut_ic,wobble,cgn_smooth,tmp
  double precision :: C_min=0.23,C_ave=0.30,C_max=.517, C_ratio
...
!Single phase
!             unk(ii,i,j,k,lb) =1.-(.5 -.5*tanh(steepness*((xi*xi+yi*yi)**0.5-nuc_radius)))/snap21


              !one Dimensional
             unk(ii,i,j,k,lb) =1.-(.5 -.5*tanh(steepness*(xi-nuc_radius)))/snap21
             unk(ii+nunkvbles,i,j,k,lb) = 0.3
!             unk(ii+nunkvbles,i,j,k,lb) = c_min+unk(ii,i,j,k,lb)*(c_ave-c_min)
!             unk(ii+nunkvbles,i,j,k,lb) = c_min+unk(ii,i,j,k,lb)*(c_ave-c_min+(c_max-c_ave)*exp(0.05*(nuc_radius-xi)))
             

             if(total_vars.eq.3)unk(ii+2*nunkvbles,i,j,k,lb) = 0d0
