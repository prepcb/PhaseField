...
!  write(*,*)"in app_mg_get_defect_var"
...
!  write(*,*)"exit app_mg_get_defect_var"
...
double precision :: unk_(M_unk),vblesd(N_unk+1,10,10),hh=1d-10
integer ip,n,ii,jj,aa
double precision :: FiM(N_unk,4),FidM(N_unk,4), unkM(M_unk,4)
double precision :: vblesM(N_unk+1,10,10,4),c,rfactor,rf4,rf5,rf6,MolPerVol


!  write(*,*)"in GET_RHS_FI_2D_D"
  ...
  do ii=1,10
     do jj=1,10
        do ip=1,N_unk
           do aa = 1,4
              vblesM(ip,ii,jj,aa)=unk(1+(ip-1)*nunkvbles,ii+rrarray(aa,1),jj+rrarray(aa,2),1,lb)
           enddo
        enddo
     enddo
  enddo
...
!set up correct sign of "h" for numerical differentiation
  ...
  if(vblesM(ifid,i,j,1).gt.0.5)then
     hh=-abs(hh)
  endif
