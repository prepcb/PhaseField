...
subroutine app_output_occasional(pid, noprocs, inumber)
  ...
  if (paraview.eq..false.) then
     call pf_output_2d(pid, inumber)
  else
...
subroutine phase_field_check(pid, inumber)
  ...
  double precision :: dx,dist,phi_val_1,phi_val_2,phi_val_3,phi_val_4,solute_max,solute_min,s1,s2,s3,s4,s5
  double precision :: new_loc, new_rad,new_DW, tempr,temp1,temp2,temp3,temp4,temp5,sol_max,sol_min,heat(4)
...
!     if(nodetype(lb).eq.1) then
        ...
        if(on_axisx)then
           ...
           if(dx.lt.1.0)call phase_loc_rad(lb, s1,s2,s3,s4,s5)
...
!  call MPI_REDUCE(temp_min,temp4,1,amr_mpi_real,MPI_MIN,0,MPI_COMM_WORLD,ierr)
  ...
  if(pid.eq.0)print *,"Tip position and radius and width"
  if(pid.eq.0)print *,temp3/temp1,temp2/temp4,1/temp5
...
subroutine  phase_loc_rad(lb, s1,s2,s3,s4,s5)
  use paramesh_dimensions
  use physicaldata
  use tree

  implicit none
  double precision, intent(inout):: s1,s2,s3,s4,s5
  integer i, j, k, lb
...
!  y1 = bnd_box(1,2,lb)+(j-1.5)*dx
  ...
  do i=il_bnd+nguard,iu_bnd-nguard
         ...
         phi_xx = 0.5*(unk(1,i+1,j,k,lb) - 2.*unk(1,i,j,k,lb) +  unk(1,i-1,j,k,lb))/(dx*dx)
...
!         dw_phi=phi**2*(1d0-phi)**2
         ...
         s3=s3+(1d0 - phi)*dx
         s4=s4+dw_phi*phi_yy
         s5=s5+ abs(phi_xx)*dx !phix at phi=1/2
...
subroutine pf_output_2d(pid, fnum)
  ...
  double precision dx
  double precision x0,y0,z0,p0,n0,xposition, FreeEnergy,potential
...
!                  if(abs(phi-0.5).lt.0.4)write(150,'(5(E16.5,X))')x0,y0,phi,T,c
                  write(150,'(5(E16.5,X))')x0,y0,phi,T,c
               endif
            end do
         end do
         !      enddo
        ...
        do k=1,9 
           do j=1,9
              do i=1,9
                 ...
                 if (minval(cell).le.0.55d0.and. maxval(cell).ge.0.45d0)then ! found cell
                    n_count=n_count+1
                 endif
              end do
           end do
