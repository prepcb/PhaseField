...
!   if(lp.eq.2)potential = potential -1d1*phi(3)
!   if(lp.eq.3)potential = potential -1d1*phi(2)


!14.678 is: solve f[2](c)=f[3](c) => c=0.807935201  (f[2](c)-f[1](c)) * molpervol(c)/W(1,1)*16 = 548*60203/3.596892202*10^7*16
! where 16 comes from 16*phi(2)^2*phi(3)^2 =1 when phi(2)=phi(3)=0.5
! this gives a well between the two solid states so that at T=delta the state preferred is solid. 

!   if(c.gt..6158704022)then
!      if(lp.eq.2)potential = potential -14.678*eps22(lp,ip)*2d0*phi(lp)*phi(3)**2!*734.8672554*(c-.6158704022)**2*(c-1d0)**2
!      if(lp.eq.3)potential = potential -14.678*eps22(lp,ip)*2d0*phi(lp)*phi(2)**2!*734.8672554*(c-.6158704022)**2*(c-1d0)**2
!   endif
...
!solute
!   if(c.gt..6158704022)then
!      potential = potential - -14.678*eps22(lp,ip)*phi(2)**2*phi(3)**2*(2939.469022*(c-.615870403037363))*(c-.807935199837264)*(c-1d0)
!   endif
endif
...
double precision :: ff,psi,phi_(3)=(/-4d0/27d0,31d0/54d0,31d0/54d0/),d_psi_d_phi(3),alpha=0d0
integer ip,n
...
psi = phi(1)*phi(2)*phi(3)*27d0*alpha 
d_psi_d_phi(1)=27d0*phi(2)*phi(3)*alpha 
d_psi_d_phi(2)=27d0*phi(3)*phi(1)*alpha 
d_psi_d_phi(3)=27d0*phi(4)*phi(2)*alpha 
ff = (1d0-psi)*FreeEnergy(c,T,Phi,0d0,0d0,lp)+psi*FreeEnergy(c,T,Phi_,0d0,0d0,lp)
ff = ff + d_psi_d_phi(lp)*(FreeEnergy(c,T,Phi_,0d0,0d0,0) - FreeEnergy(c,T,Phi,0d0,0d0,0))

ff = FreeEnergy(c,T,Phi,0d0,0d0,lp)

 phaseRHS=M_tilde*(GradientEnergy(LapPhi,vbles,lp,dx)-potential(Phi,c,lp)*scale_factor**2/lambda**2&
      -scale_factor**2/lambda*ff/W_a(1,1))
...
double precision :: nu_Pb=54730.,nu_Sn=61505.,psi,phi_(3)=(/-4d0/27d0,31d0/54d0,31d0/54d0/)
...
!strictly must have entropymixing term
...
! Folch Plapp
  FreeEnergy = FreeEnergy+(hb-ha+hr)*(f_phi(phi,i,0,0)+psi*(f_phi(phi_,i,0,0)-f_phi(phi,i,0,0)))
 enddo
! add in other factor and non dimensionalise
