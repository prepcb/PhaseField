...
! parameters for Ni-Cu
  ...
  double precision :: g_L(2)=(/2.35e9,1.728e9/),g_delta(2)=(/2.2e-9 ,2.2e-9 /),g_beta(2)=(/0.0033,0.0039/),g_epsilon(2)
  double precision :: g_C(2)=(/5.42e6,3.96e6/),g_MolPerVol(2)=(/1.517d5,1.406d5/),g_D(2)=(/1e-9,1e-13/) !liquid, solid
  double precision :: g_tau,g_lengthSQ,g_Dch, g_R = 8.31,g_T0=1549.15 !J/(K mol)
  logical :: heat_c_term=.true.,heat_phi_term=.true.,grad_DW_term=.true.
  double precision :: heat_size(4)=0d0,g_c0=0.5,g_D_tilde=1d2
