...
use solution_parameters
implicit none
double precision, intent (in):: dx,c,T,phi(N_phases),c_dot,phi_dot(N_phases),vbles(N_unk+1,3,3,3)
...
! to help with floating point arithmetic
  ...
  do ii=1,3
     do jj=1,3
        do kk=1,3
        T_=vbles(N_phases+1,ii,jj,kk)*stencil3d(ii,jj,kk)
...
double precision :: c_dot=0d0,phi_dot(1)=0d0,beta,phi_
...
use solution_parameters
implicit none
double precision, intent(in)::dx,c,T,Phi(N_phases),vbles(N_unk+1,3,3,3)
integer, intent(in):: lp
...
Fi=0d0
  ...
  if (mode_1.eq.1) then
     phi_dot(1) = (vbles(1,2,2,2)-unk_(2))/dt
     n=2*nunkvbles
     c_dot = (vbles(3,2,2,2)-unk_(2+n))/dt
  elseif(mode_1.eq.2)then
     ...
     phi_dot(1) = (rf4*unk_(5)-rf5*unk_(4)+rf6*vbles(1,2,2,2))/dt

     n=2*nunkvbles
     c_dot = (rf4*unk_(5+n)-rf5*unk_(4+n)+rf6*vbles(3,2,2,2))/dt
  else
    ...
    do ii=1,3
       do jj=1,3
          do kk=1,3
             ...
             LapPhi(ip)=LapPhi(ip)+stencil3d(ii,jj,kk)*vbles(ip,ii,jj,kk)/(dx*dx)
