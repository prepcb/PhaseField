...
!isotropic                                                                                                
  ...
  enddo
!  if(lp.eq.1)     GradientEnergy = GradientEnergy + LapPhi(lp)
...
!   if(lp.eq.1)potential = potential + 2d0*phi(1)-4d0*phi(1)**3
!   if(lp.eq.1)potential=potential +12d0*phi(1)*phi(2)*phi(3)
!   if(lp.eq.2)potential=potential +6d0*phi(3)*phi(1)**2
!   if(lp.eq.3)potential=potential +6d0*phi(1)**2*phi(2)
...
!      -scale_factor**2/lambda*FreeEnergy(c,T,Phi,0d0,0d0,lp)/W_a(1,1))

 phaseRHS=M_tilde*(GradientEnergy(LapPhi,vbles,lp,dx)-potential(Phi,c,lp)*scale_factor**2/lambda**2)

 phaseRHS=phaseRHS+1d0/lambda*FreeEnergy(c,T,Phi,0d0,0d0,lp)/W_a(1,1)
...
double precision TemperatureRHS,SoluteRHS,T_k,phi_,w(4)
logical Not_number
integer ip,n,ii,jj,kk,iter
...
!!!!!!!!!!!!!!!!
  ...
  Fi(N_phases+1) = 0d0!TemperatureRHS(vbles,dx,c,T,phi,c_dot,phi_dot)
...
double precision :: nu_Pb=54730.,nu_Sn=61505.,psi,dpsi(3,3,3),f_mix,ff(3),c_,alpha=-0.2d0
...
psi=f_phi(phi,1,0,0)+(1d0-f_phi(phi,1,0,0))*&
     (f_phi(phi,1,0,0)*f_phi(phi,2,0,0)+f_phi(phi,2,0,0)*f_phi(phi,3,0,0)+f_phi(phi,3,0,0)*f_phi(phi,1,0,0))



dpsi(lp,1,2) = f_phi(phi,1,lp,0)*f_phi(phi,2,0,0) + f_phi(phi,1,0,0)*f_phi(phi,2,lp,0)
dpsi(lp,2,3) = f_phi(phi,2,lp,0)*f_phi(phi,3,0,0) + f_phi(phi,2,0,0)*f_phi(phi,3,lp,0)
dpsi(lp,3,1) = f_phi(phi,3,lp,0)*f_phi(phi,1,0,0) + f_phi(phi,3,0,0)*f_phi(phi,1,lp,0)
...
! enddo
 ...
 if(lp.eq.0)then !free energy proper
    ...
    enddo
    FreeEnergy = FreeEnergy/(R*T_eut)! + R*Tk*EntropyMixing(c,0)
    
 elseif(lp.le.N_phases)then !phase = current_var
! Free energy is

!(ff(1)*f_phi(phi,1,0,0)+ff(2)*f_phi(phi,2,0,0)+ff(3)*f_phi(phi,3,0,0)&
!     + f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)&
!     + f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)&
!     + f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut))/psi

! the following differentiates with respect to phi(lp)
    ...
    do i=1,N_phases
       ...
       ff(i) = (c*hb+(1.-c)*ha+hr)*MolPerVol
       FreeEnergy = FreeEnergy +ff(i)*f_phi(phi,i,lp,0)*f_phi(phi,1,0,0)
       ff(i)=ff(i)+MolPerVol*(1d0+T)*EntropyMixing(c,0)*R*T_eut
    enddo
    FreeEnergy = FreeEnergy + f_phi(phi,1,lp,0)*&
         (f_phi(phi,1,0,0)*ff(1)  + f_phi(phi,2,0,0)*ff(2)  + f_phi(phi,3,lp,0)*ff(3))
    
    !Order of tangents : 2, 1 ,3
    
    

    FreeEnergy = FreeEnergy + (1d0-f_phi(phi,1,0,0))*dpsi(lp,3,1)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)
    FreeEnergy = FreeEnergy + (1d0-f_phi(phi,1,0,0))*dpsi(lp,1,2)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)
    FreeEnergy = FreeEnergy + (1d0-f_phi(phi,1,0,0))*dpsi(lp,2,3)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut)
    
    FreeEnergy = FreeEnergy/psi

    FreeEnergy = FreeEnergy -(&
           f_phi(phi,1,0,0)*(ff(1)*f_phi(phi,1,0,0)+ff(2)*f_phi(phi,2,0,0)+ff(3)*f_phi(phi,3,0,0))&
     + (1d0-f_phi(phi,1,0))*(f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)&
                           + f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)&
                           + f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*f_mix(c,c23,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut)))&
     /psi**2*(f_phi(phi,1,lp,0)*(1d0-f_phi(phi,2,0,0)*f_phi(phi,3,0,0)-f_phi(phi,3,0,0)*f_phi(phi,1,0,0)-f_phi(phi,1,0,0)*f_phi(phi,2,0,0))&
     + (1d0-f_phi(phi,1,0,0))*(dpsi(lp,2,3)+dpsi(lp,3,1)+dpsi(lp,1,2)))
    return
...
!ff(1)*f_phi(phi,1,0,0)+ff(2)*f_phi(phi,2,0,0)+ff(3)*f_phi(phi,3,0,0)&
!     + 4d0*f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),MolPerVol*R*T_eut)&
!     + 4d0*f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),MolPerVol*R*T_eut)&
!     + 4d0*f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),MolPerVol*R*T_eut)/psi

! the following differentiates with respect to c
 FreeEnergy = 0.!debug
 call GibbsVector(V,Tk,0)
...
! Folch Plapp
  ff(i) = (hb-ha+hr)/(R*T_eut) ! make non dimensional
  ff(i)=ff(i)+(1d0+T)*EntropyMixing(c,1)
  FreeEnergy = FreeEnergy+ff(i)*f_phi(phi,i,0,0)*f_phi(phi,1,0,0)
!  FreeEnergy = FreeEnergy + (1d0+T)*EntropyMixing(c,1)
 enddo
! lamda is already non-dimensional
 FreeEnergy = FreeEnergy + (1d0-f_phi(phi,1,0,0))*(&
      + f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*lamda(3,1)&
      + f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*lamda(1,2)&
      + f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*lamda(2,3))
 FreeEnergy = FreeEnergy/psi
...
!  enddo
    ...
    do i=1,N_phases
       ...
       ff(i) = (c*hb+(1.-c)*ha+hr)
       do j=1,N_phases
          FreeEnergy = FreeEnergy +ff(i)*f_phi(phi,i,j,0)*phi_dot(j) 
       enddo
       ff(i)=ff(i)+(1d0+T)*EntropyMixing(c,0)*R*T_eut
    enddo
...
!    do i=1,N_phases
!       FreeEnergy = FreeEnergy + dpsi_dphi(i,3,1)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),R*T_eut)*phi_dot(i)
!       FreeEnergy = FreeEnergy + dpsi_dphi(i,1,2)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),R*T_eut)*phi_dot(i)
!       FreeEnergy = FreeEnergy + dpsi_dphi(i,2,3)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),R*T_eut)*phi_dot(i)
       
!       FreeEnergy = FreeEnergy/psi
!       write(*,*)"in free energy temperature"
!       stop
!       FreeEnergy = FreeEnergy -(ff(1)*f_phi(phi,1,0,0)+ff(2)*f_phi(phi,2,0,0)+ff(3)*f_phi(phi,3,0,0)&
!            + alpha*4d0*f_phi(phi,3,0,0)*f_phi(phi,1,0,0)*f_mix(c,c31,lamda(3,1),ff(1),ff(3),R*T_eut)&
!            + alpha*4d0*f_phi(phi,1,0,0)*f_phi(phi,2,0,0)*f_mix(c,c12,lamda(1,2),ff(2),ff(1),R*T_eut)&
!            + alpha*4d0*f_phi(phi,2,0,0)*f_phi(phi,3,0,0)*f_mix(c,c12,lamda(2,3),ff(2),ff(3),R*T_eut))&
!            /psi**2*(dpsi_dphi(i,3,1)+dpsi_dphi(i,1,2)+dpsi_dphi(i,2,3))*phi_dot(i)
!    enddo
    FreeEnergy = FreeEnergy*Tk
...
!    pure -dF/dphi term  
    ...
    do i=1,N_phases
       ...
       ff(i) = (c*hb+(1.-c)*ha+hr)
       do j=1,N_phases
          FreeEnergy = FreeEnergy +ff(i)*f_phi(phi,i,j,0)*phi_dot(j) !a bit clumsy but ff(i) needs to be full component Free Energy
          ff(i)=ff(i)+(1d0+T)*EntropyMixing(c,0)*R*T_eut
       enddo
    enddo
...
! add in other factor and non dimensionalise                                                                                                                                                                                                 
! Folch Plapp                                                                                                                                                                                                                                
  FreeEnergy = FreeEnergy+(hr)*f_phi(phi,i,0,0)
 enddo
! add in other factor and non dimensionalise                                                                                                                                                                                                 
...
! Folch Plapp
  ff(i) = (hb-ha+hr)/(R*T_eut) ! make non dimensional
  FreeEnergy = FreeEnergy+ff(i)*f_phi(phi,i,0,0)
...
double precision function f_mix(c,cc,slope,f_left,f_right,factor)
double precision, intent (in)::c,cc(2),slope,f_left,f_right,factor
double precision :: FreeEnergy

f_mix=0d0
if(c.lt.cc(1))then
   f_mix=f_left
elseif(c.gt.cc(2))then
   f_mix=f_right
else
   f_mix=cc(3) + (c-cc(1))*slope
   f_mix=f_mix*factor
endif
