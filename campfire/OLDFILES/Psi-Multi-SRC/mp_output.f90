...
subroutine phase_field_check(pid, inumber)
  ...
  double precision :: phase,umin,umax,phimin,phimax,u,phi(n_phases),c,T,temp,gradphi,gradphi2,theta1,theta2
     ...
     n_leaf=n_leaf+1
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
              ...
              TotalEnergy = TotalEnergy + GradientEnergy(vbles,0,dx)*dx*dx&
                   +FreeEnergy(c,T,phi,0d0,phi,0)*MolPerVol/(g_W(1)*g_lambda**2)*dx*dx&
                   +potential(Phi,c,0)/g_lambda**2*dx*dx
...
!  print *,"Phase_field_check axis node count:",axis_node_count
  ...
  call MPI_REDUCE(abs(s4)+1d-15,temp5,1,amr_mpi_real,MPI_SUM,0,MPI_COMM_WORLD,ierr)
...
!  call MPI_REDUCE(dist,temp,1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)
  ...
  if(pid.eq.0)print *,"Tip position and radiusq",temp2/temp4,temp3/temp5,TotalEnergy0
  if(pid.eq.0)print *,"dt",dt
  if(pid.eq.0)write(126,'(7e15.7)') time, temp2/temp4,temp3/temp5,solute_min,solute_max,solute_min/solute_max,TotalEnergy0
...
subroutine  phase_loc_rad(lb, s1,s2,s3,s4)
  ...
  double precision dw_phi,phi_psi,psi(2),phi_(3)
 integer isng
...
!     if (unk(1,i,j,k,lb).gt.1d-5.and.unk(1,i,j,k,lb).lt.1d0-1d-5) then
         ...
         psi(1)=unk(1,i+1,j,k,lb)
         psi(2)=unk(1+6,i+1,j,k,lb)
         phi=phi_psi(psi,1)
         psi(1)=unk(1,i-1,j,k,lb)
         psi(2)=unk(1+6,i-1,j,k,lb)
         phi_x=0.5*(phi-phi_psi(psi,1))/dx

         psi(1)=unk(1,i,j,k,lb)
         psi(2)=unk(1+6,i,j,k,lb)
         phi=phi_psi(psi,1)
         psi(1)=unk(1,i,j+1,k,lb)
         psi(2)=unk(1+6,i,j+1,k,lb)
         phi_yy=(-phi+phi_psi(psi,1))/(dx*dx)
...
subroutine pf_output_2d(pid, fnum)
  ...
  double precision dx
  double precision x0,y0,p0,n0,xposition, FreeEnergy,potential,Fi(N_unk)
  double precision ::c,T,phi(n_phases),psi(2),c_dot,phi_dot(3),rfactor,rf4,rf5,rf6,K_heat(4)
  double precision :: phi_psi
      ...
      dx = bsize(1,lb)/real(nxb)
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
              ...
              psi(1)=unk(1+0*nunkvbles,i,j,1,lb)
              psi(2)=unk(1+1*nunkvbles,i,j,1,lb)
              ...
              write(150,'(7(E16.5,X))')x0,y0,phi_psi(psi,1),phi_psi(psi,2),c,psi(1),psi(2)
