...
subroutine app_parameter_set
  ...
  verbose=0              ! How much terminal output
  ...
  C2F_switch = 0
  allsolid_test = 1      ! Derefinement if all of a block is over 0.999
  smooth_count_max = 3   ! seems better for 3D runs
  defect_tol_max = 3e-5
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  epsilon = 0.02
  ke = 0.30
  le = 40.0                  ! Only used if thermal-solutal coupled
  mcinf = 0.05	             ! Only used if thermal-solutal coupled
  solute=.false.             ! Only used if not coupled
  ...
  call input_parameter_file_reading()
write(*,*) solute
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  double precision :: grid_min=-200.0,grid_max=200.0     ! grid 7
  double precision :: grid_min=-100.0,grid_max=400.0     ! grid 6   !Recommended coarse block resolution
...
!              neigh(2,:,lnblocks) = 0
  ...
  if (eighthdmn.eq.1) then  ! eighthdmn =1 means running 1/8 of the domain
