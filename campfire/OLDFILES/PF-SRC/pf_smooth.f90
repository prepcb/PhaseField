  ...
  if (multigrid_on.ge.1) then
     do lbs=block_starts(level), block_starts(level+1)-1
        ...
        do current_var=1,total_vars
           ...
           call pf_mg_smooth_var(lb,F_i)  !! Fi is d_p^{n+1,m}
        enddo
        do current_var=1,total_vars
           current_unk=1+(current_var-1)*nunkvbles
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine amr_mg_get_Energy(pid, Energy)
  use multigrid_parameters
  use paramesh_dimensions
  use time_dep_parameters
  use solution_parameters
  ...
  integer :: i,j,k,lb
  double precision :: dx, LapMult, tau_val,dx2inv,Fi
  ...
  do lb= 1,lnblocks
     if(.not.has_children(lb))then     
        ...
        do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d 
           do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d 
              do i=il_bnd+nguard,iu_bnd-nguard
                 call get_stencil_2d_Energy(i,j,dx,lb,Fi)
                 Energy = Energy + Fi
              end do
           end do
        end do
  ...
  enddo
end subroutine  amr_mg_get_Energy
