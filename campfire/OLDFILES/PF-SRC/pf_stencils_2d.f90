...
subroutine get_stencil_2d_energy(i,j,dx,lb,Fi)
  use solution_parameters
  use multigrid_parameters
  use physicaldata
  ...
  implicit none
  include 'mpif.h'
  integer, intent(in) :: i,j,lb
  double precision, intent(in) :: dx
  double precision, intent(out) :: Fi
  double precision :: phix,phiy
  double precision :: A_func,gradphisq
  double precision :: phi,U,c
  double precision :: alewis
  double precision :: A_func2d
  integer k
  k=1
  ...
  gradphisq=phix*phix+phiy*phiy       
  call get_w_aniso_2_2d(phix,phiy,gradphisq,A_func)      
  Fi=dx*dx*(0.5*A_func*A_func*gradphisq + 0.5*phi**2 + 0.25*phi**4 + lambda*(phi-2.0/3.0*phi**3+0.2*phi**5)*(U + mcinf*c)) !energy


end subroutine get_stencil_2d_Energy
