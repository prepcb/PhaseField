...
! if(pid.eq.0)write(*,*) "smoother inside begin"
! if(pid.eq.0)write(*,*) il_bnd,iu_bnd,jl_bnd,ju_bnd,kl_bnd,ku_bnd,lnblocks
...
subroutine pf_mg_smooth_var(lb)
  ...
  use tree
  implicit none
!  double precision, intent(out)::F_i(il_bnd:iu_bnd,jl_bnd:ju_bnd,kl_bnd:ku_bnd,3,lnblocks)
  ...
  do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d 
     do j=jl_bnd+nguard*k2d,ju_bnd-nguard*k2d 
        do i=il_bnd+nguard,iu_bnd-nguard
           ...
           unk(current_unk,i,j,k,lb) = unk(current_unk,i,j,k,lb)-weight*Fi/dFi_dvi
