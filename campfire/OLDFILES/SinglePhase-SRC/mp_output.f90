...
subroutine phase_field_check(pid, inumber)
  ...
  double precision :: new_loc, new_rad, tempr,temp2,temp3,temp4,temp_max,temp_min,phi_dot_sq,max_entropy
...
!        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
              xl = bnd_box(1,1,lb)+(i-1.5)*dx
              if(unk(1,i,1,2,lb).gt.0.01)then
                 temp_max=max(unk(1+nunkvbles,i,2,1,lb),temp_max)
                 temp_min=min(unk(1+nunkvbles,i,2,1,lb),temp_min)
              end if
           enddo
!        enddo
!     endif
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
              phi_dot_sq = max(((unk (1, i, j, k, lb)-unk (2, i, j, k, lb))/dt),phi_dot_sq)
           enddo
        enddo
     end if
  end do
!  print *,"Phase_field_check axis node count:",axis_node_count
  ...
  call MPI_REDUCE(phi_dot_sq,max_entropy,1,amr_mpi_real,MPI_MAX,0,MPI_COMM_WORLD,ierr)

  if(pid.eq.0)write (6,'(A,A)') 27, "[36;1;47m"
!  if(pid.eq.0)print *,"Tip Radius",temp3
  if(pid.eq.0)write(*,'(A12,F12.5)')"solute ratio",temp4/temp3
!  if(time.gt.time_march)then
!     if(pid.eq.0)write(unit=125,fmt='(e12.5,A,e12.5,A,e12.5)') time,',', temp2,',',tempr
!  endif
  ...
  if(temp2.gt.8d2)then
     write(*,*)"position too large > 800"
     stop
  end if
  if(pid.eq.0)write(*,'(A12,F12.5)')"position    ",temp2
  if(pid.eq.0)write(*,'(A12,F12.5)')"radius      ",tempr
  if(pid.eq.0)write(*,'(A12,F12.5)')"time        ",time
  if(time.gt.time_march+1d1)then
     time_march=time
  endif

  if(time.gt.time_march)then
     if(pid.eq.0)write(unit=126,fmt='(e12.5,A,e12.5,A,e12.5,A,e12.5)') time,',', temp2,',', tempr,',',temp4/temp3
     time_march=time_march+1d1
  endif
...
!         b5 =  dx*atanh(unk(1,i,j,k,lb))/(atanh(unk(1,i,j,k,lb))-atanh(unk(1,i+1,j,k,lb)))
  ...
  cur_loc=x0+b5
 ! phi_yy=max(phi_yy,1d-10)
  cur_rad = 0d0!abs(phi_x/phi_yy)+b5  
...
subroutine pf_output_2d(pid, fnum)
  ...
  double precision ::c,T,phi,c_dot,phi_dot,rfactor,rf4,rf5,rf6
  ...
  do lb=1,lnblocks
     if(nodetype(lb).eq.1) then
        ...
        do j=jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
                 if (ABS(unk(1,i,j,1,lb)).lt.1.0e-20) unk(1,i,j,1,lb)=0.0
           end do
        end do
     end if
  end do
  ...
  do lb=1,lnblocks
    if(.not.has_children(lb)) then
      dx = bsize(1,lb)/real(nxb)
        do j=2,2!jl_bnd+1,ju_bnd-1
           do i=il_bnd+1,iu_bnd-1
             x0 =  bnd_box(1,1,lb)+(i-1.5)*dx
             y0 =  bnd_box(1,2,lb)+(j-1.5)*dx
                write(150,'(4(E16.5,X))')x0,y0,unk(1,i,j,1,lb),unk(1+nunkvbles,i,j,1,lb)
