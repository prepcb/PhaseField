...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      Subroutine sg_guardcell(mype,iopt,mglevel,         & 
                               nlayersx,nlayersy,nlayersz)
...
!!--tim         If (.not.present(nlayersx)) Then
            nlayers0x = nguard
!!--tim         Else
!!--tim            nlayers0x = nlayersx
!!--tim         End if
!!--tim         If (.not.present(nlayersy)) Then
            nlayers0y = nguard
!!--tim         Else
!!--tim            nlayers0y = nlayersy
!!--tim         End if
!!--tim         If (.not.present(nlayersz)) Then
            nlayers0z = nguard
!!--tim         Else
!!--tim            nlayers0z = nlayersz
!!--tim         End if
!!--tim      Else
!!--tim         If (.not.present(nlayersx)) Then
!!--tim            nlayers0x = nguard_work
!!--tim         Else
!!--tim            nlayers0x = nlayersx
!!--tim         End if
!!--tim         If (.not.present(nlayersy)) Then
!!--tim            nlayers0y = nguard_work
!!--tim         Else
!!--tim            nlayers0y = nlayersy
!!--tim         End if
!!--tim         If (.not.present(nlayersz)) Then
!!--tim            nlayers0z = nguard_work
!!--tim         Else
!!--tim            nlayers0z = nlayersz
!!--tim         End if
!!--tim      End if ! End If (iopt == 1)
...
!-----Restrict solution to parent blocks
        If (.not.advance_all_levels) Then
          iempty = 0
!          if (mype.eq.1) print*,'pf_restrict',mglevel,sum(commatrix_recv(:))
!!--tim          call pf_restrict(mype,iopt,iempty,.True.,mglevel)
          call amr_1blk_guardcell_reset
        End if
